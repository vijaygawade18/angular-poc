...
function drawPath(svg, path, startX, startY, endX, endY) {
    ...
    var deltaX = (endX - startX) * 0.10;
    var deltaY = (endY - startY) * 0.10;
    // for further calculations which ever is the shortest distance
    var delta = deltaY < absolute(deltaX) ? deltaY : absolute(deltaX);

    if (svg.attr("height") < endY) svg.attr("height", 500)//endY + delta);
    if (svg.attr("width") < (startX + stroke)) svg.attr("width", 500)//(startX + stroke + delta));
    if (svg.attr("width") < (endX + stroke)) svg.attr("width", 500);//(endX   + stroke + delta));
